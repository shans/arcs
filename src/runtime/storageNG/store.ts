...
import { MuxEntityType, SingletonEntityType, NonMuxType, SingletonInterfaceStore, CRDTTypeRecordToType, TypeToCRDTTypeRecord, CRDTMuxEntity, CRDTEntitySingleton, NonMuxCRDTTypeRecord } from './storage-ng.js';
...
export class Store<T extends CRDTTypeRecord> extends AbstractStore<CRDTTypeRecordToType<T>> implements StoreInterface<T> {
  ...
  private activeStore: ActiveStore<CRDTTypeRecordToType<T>, T> | null;
  ...
  async activate(): Promise<ActiveStore<CRDTTypeRecordToType<T>, T>> {
    let a : AbstractStore<Type>;
    let aa : AbstractStore<MuxEntityType>;
    ...
    let d : ToActiveStore<Type>;
    let e : ActiveStore<SingletonEntityType, CRDTEntitySingleton>
    d = e
    e = d

    // what we have: 
    //
    // AbstractStore<Type>.activate -> ActiveStore
    // AbstractStore<MuxEntityType>.activate -> ActiveMuxer
    //
    // Store.activate -> ActiveStore
    // StoreMuxer.activate -> ActiveMuxer
    //
    //

    // AbstractStore<Type>.activate => AbstractActiveStore
    // AbstractStore<NonMuxType>.activate => ActiveStore (extends AbstractActiveStore)
    // 
    ...
    const activeStore = await constructor.construct<Type>({
    ...
    }) as ActiveStore<CRDTTypeRecordToType<T>, T>;
...
export class StoreMuxer<T extends TypeToCRDTTypeRecord<MuxEntityType>> extends AbstractStore<MuxEntityType> implements StoreInterface<T> {
  protected unifiedStoreType: 'Store' = 'Store';
  ...
  constructor(type: MuxEntityType, opts: StoreInfo & {storageKey: StorageKey, exists: Exists}) {
    super(opts);
    this.type = type;
    this.storageKey = opts.storageKey;
    this.exists = opts.exists;
  ...
  async activate(): Promise<ActiveMuxer<T>> {
    ...
    const activeStore = await constructor.construct<MuxEntityType>({
      storageKey: this.storageKey,
      exists: this.exists,
      type: this.type,
      mode: this.mode,
